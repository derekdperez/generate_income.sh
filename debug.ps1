...
.DESCRIPTION
  Execute from the repository root: .\debug.ps1
...
if ($results.Count -eq 0) {
    ...
    # Ensure JSON root is always an array (ConvertTo-Json edge cases with single object).
    $wrapped = , @($results.ToArray())
    $json = $wrapped | ConvertTo-Json -Depth 10

# AI agent debugging workflow (repo-based)

This repository includes a **machine-run diagnostic harness**. Future AI agents should use it instead of asking the human to copy/paste individual shell commands from chat.

## What to do (agents)

1. **Do not** ask the user to manually run one-off diagnostic commands from chat.
2. **Add** the diagnostic commands you need to:
   - `.ai-debug/debug_commands.sh` (Unix / macOS / Linux), and/or  
   - `.ai-debug/debug_commands.ps1` (Windows PowerShell)  
...
6. Tell the user to run **from the repo root**:
...
7. Tell the user to paste the contents of **`debug_results.json`** back into the chat (or attach the file).
...
9. Prefer **narrowly scoped** commands (e.g. `docker compose ps`, `docker logs --tail 50 <service>`).
10. **Remove** obsolete diagnostic commands when they are no longer needed so the files stay small and clear.
11. Keep commands **reproducible** and easy to understand; add `#` comments where helpful.
...
2. From the **repository root**, run `./debug.sh` or `.\debug.ps1`.
3. Open `debug_results.json` and paste its contents (or attach it) into the chat.
...
Repo root also has **`debug.sh`** and **`debug.ps1`** runners.

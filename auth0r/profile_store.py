...
WHERE p.root_domain = %s
  AND p.enabled = TRUE
  AND i.enabled = TRUE
ORDER BY p.created_at_utc ASC, i.created_at_utc ASC;
"""
        out: list[AuthIdentity] = []
        with self._connect() as conn:
            with conn.cursor() as cur:
                cur.execute(sql, (rd,))
                rows = cur.fetchall()
                for row in rows:
                    login_cfg = _json_load(decrypt_text(row[8]), {})
                    custom_headers = _json_load(decrypt_text(row[9]), {})
                    imported_cookies = self._list_cookie_jars(cur, row[1])
                    out.append(
                        AuthIdentity(
                            id=row[1],
                            profile_id=row[0],
                            ...
                            username=decrypt_text(row[6]),
                            password=decrypt_text(row[7]),
                            ...
                            allowed_hosts=[str(v).lower() for v in (row[10] or [])],
                            success_markers=[AuthVerificationMarker(kind=str(m.get("kind","text")), value=str(m.get("value",""))) for m in (row[11] or []) if isinstance(m, dict)],
                            denial_markers=[AuthVerificationMarker(kind=str(m.get("kind","text")), value=str(m.get("value",""))) for m in (row[12] or []) if isinstance(m, dict)],
    ...
    def _list_cookie_jars(self, cur: Any, identity_id: str) -> list[dict[str, Any]]:
        cur.execute(
            "SELECT cookies_json_encrypted, host_scope_json, path_scope_json FROM auth0r_cookie_jars WHERE identity_id = %s ORDER BY created_at_utc ASC",
            (identity_id,),
        )
        out = []
        for cookies_enc, host_scope, path_scope in cur.fetchall():
            cookies = _json_load(decrypt_text(cookies_enc), [])
            if isinstance(cookies, list):
                out.extend(cookies)
        return out

    def save_runtime_session(self, root_domain: str, identity_id: str, source_type: str, generation_number: int, *, verified: bool, verification_summary: dict[str, Any], cookies: list[dict[str, Any]], auth_headers: dict[str, str]) -> str:
        session_id = str(uuid.uuid4())
        with self._connect() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    """
                    INSERT INTO auth0r_runtime_sessions(id, root_domain, identity_id, source_type, generation_number, verified, verification_summary_json, cookies_encrypted, auth_headers_encrypted)
                    VALUES (%s, %s, %s, %s, %s, %s, %s::jsonb, %s, %s)
                    """,
                    (
                        session_id,
                        str(root_domain).lower(),
                        identity_id,
                        source_type,
    ...
    def save_recorded_action(self, root_domain: str, identity_id: str, runtime_session_id: str, action: dict[str, Any]) -> str:
        ...
        with self._connect() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    """
                    INSERT INTO auth0r_recorded_actions(id, root_domain, identity_id, runtime_session_id, url, method, source, content_type, likely_state_changing, metadata_json)
                    ...
                    (
                        ...
                        runtime_session_id,
                        str(action.get("url", "")),
                        str(action.get("method", "GET")),
                        str(action.get("source", "nightmare_seed")),
                        str(action.get("content_type", "")),
                        bool(action.get("likely_state_changing", False)),
    ...
    def save_replay_attempt(self, root_domain: str, identity_id: str, recorded_action_id: str, replay_variant: str, *, status_code: int | None, suspicious: bool, summary: dict[str, Any]) -> str:
        replay_id = str(uuid.uuid4())
        with self._connect() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    """
                    INSERT INTO auth0r_replay_attempts(id, root_domain, identity_id, recorded_action_id, replay_variant, status_code, suspicious, summary_json)
                    VALUES (%s, %s, %s, %s, %s, %s, %s, %s::jsonb)
                    """,
                    (replay_id, str(root_domain).lower(), identity_id, recorded_action_id, replay_variant, status_code, bool(suspicious), json.dumps(summary or {})),
                )
            conn.commit()
        return replay_id

    def save_finding(self, root_domain: str, identity_id: str | None, finding_type: str, severity: str, title: str, endpoint: str, replay_variant: str, confidence: float, summary: dict[str, Any]) -> str:
        finding_id = str(uuid.uuid4())
        with self._connect() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    """
                    INSERT INTO auth0r_findings(id, root_domain, identity_id, finding_type, severity, title, endpoint, replay_variant, confidence, summary_json)
                    VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s::jsonb)
                    """,
                    (finding_id, str(root_domain).lower(), identity_id, finding_type, severity, title, endpoint, replay_variant, float(confidence), json.dumps(summary or {})),
                )
            conn.commit()
        return finding_id
